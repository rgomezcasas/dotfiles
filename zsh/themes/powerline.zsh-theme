...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{red}%K{white}"$'\ue0b0'"%F{white}%F{black}%K{white}"$'$(git_prompt_info)$(git_prompt_status)%F{white}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{red}%K{white}"$'\ue0b0'"%F{white}%F{black}%K{white}"$'$(git_prompt_info)%F{white}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
...
if [ $(id -u) -eq 0 ]; then
    POWERLINE_SEC1_BG=%K{9}
    POWERLINE_SEC1_FG=%F{9}
else
    POWERLINE_SEC1_BG=%K{9}
    POWERLINE_SEC1_FG=%F{9}
fi
...
PROMPT="$POWERLINE_SEC1_BG$POWERLINE_SEC1_TXT $POWERLINE_USER_NAME %k%f$POWERLINE_SEC1_FG%K{red}"$'\ue0b0'"%k%f%F{white}%K{red} "$POWERLINE_CURRENT_PATH"%F{red}"$POWERLINE_GIT_INFO_LEFT" %k"$'\ue0b0'"%f "
